...
> = {
  ko: {
    ...
    description: "시장 요약 아래에 움직임, 시그널, 빠른 이동 경로를 묶어두었습니다.",
  ...
  en: {
    ...
    description: "We bundle movement, signals, and fast routes below the market overview.",
...
export default function HomeDashboardExtras() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setEvents(Array.isArray(eventJson?.items) ? eventJson.items.slice(0, 2) : []);
        setSnapshot({
  ...
  const strongestAsset = useMemo(() => {
    return [...snapshot.assets]
      .filter((a) => typeof a.changePercent === "number")
  ...
  const marketBreadth = useMemo(() => {
    const rising = snapshot.assets.filter((a) => typeof a.changePercent === "number" && a.changePercent > 0).length;
    const falling = snapshot.assets.filter((a) => typeof a.changePercent === "number" && a.changePercent < 0).length;
  ...
  const stockHeatmapAssets = useMemo<HeatmapAsset[]>(() => {
    const benchmarks = snapshot.assets
      .filter((a) => a.symbol === "S&P 500" || a.symbol === "NASDAQ")
      .map((a) => ({ symbol: a.symbol, name: a.symbol, price: a.price, changePercent: a.changePercent, currency: a.currency || "USD" }));
    const stocks = watchlist
      .filter((item) => item.group === "stock" && typeof item.changePercent === "number")
      .sort((a, b) => Math.abs(b.changePercent || 0) - Math.abs(a.changePercent || 0))
      .map((item) => ({ symbol: item.symbol, name: getLocalizedAssetName(item, language), price: item.price, changePercent: item.changePercent, currency: "USD" }));
    return [...benchmarks, ...stocks].slice(0, 8);
  ...
  const todayScenario = useMemo(() => {
    if (snapshot.fearGreed && snapshot.fearGreed.value <= 30) {
      return {
        ...
        focus: (nextEvent ? getLocalizedEventTitle(nextEvent, language) : "") ||
          (language === "ko" ? "고베타 자산의 급격한 흔들림을 우선 체크하세요." : "Watch for sudden risk-off moves in high-beta assets.")
      };
    }
    if (marketBreadth.rising >= marketBreadth.falling + 2) {
      return {
        ...
        focus: strongestAsset?.symbol
          ? language === "ko" ? `${strongestAsset.symbol} 중심의 상대강도 흐름을 계속 보세요.` : `Momentum focus remains on ${strongestAsset.symbol}.`
          : language === "ko" ? "상대적으로 강한 자산부터 확인해 보세요." : "Lean on leaders with the cleanest upside structure."
      };
    }
    return {
      ...
      summary: language === "ko"
        ...
        : "The tape looks balanced, so macro headlines may decide the next move.",
      focus: nextEvent
        ? language === "ko" ? `주요 촉매: ${getLocalizedEventTitle(nextEvent, language)}` : `Primary catalyst: ${getLocalizedEventTitle(nextEvent, language)}`
        : language === "ko" ? "다음 예정된 거시 이벤트를 확인해 보세요." : "Monitor the next scheduled macro event for direction."
    };
  }, [language, snapshot.fearGreed, marketBreadth, nextEvent, strongestAsset]);

  const toneClass = (tone: DashboardSignal["tone"]) => {
  ...
  return (
    <section className={styles.root}>
          ...
          {signals.map((signal) => (
            <div key={signal.id} className={styles.signalCard}>
              <div className={styles.signalCardTop}>
                ...
                <span className={`${styles.signalTone} ${toneClass(signal.tone)}`}>{signal.tone}</span>
              </div>
              <p className={styles.signalCardText}>{signal.summary}</p>
            </div>
          ))}
          {!signals.length && <p className={styles.empty}>{copy.signalsLoading}</p>}
        </div>

        {/* Heatmaps + scenario - 2 column */}
        <div className={styles.visualGrid}>
          <div className={styles.heatmapStack}>
            <div className={styles.heatmapCard}>
              <div className={styles.panelHeader}>
                <h4 className={styles.visualTitle}>{language === "ko" ? "한국주식 히트맵" : "Korean stock heatmap"}</h4>
                <span className={styles.panelSubtle}>{copy.leaders}</span>
              </div>
              <div className={styles.heatmapGrid}>
                {koreaHeatmapAssets.map((asset, i) => (
                  <div key={asset.symbol} className={`${styles.heatTile} ${getHeatmapSizeClass(i)} ${getHeatmapToneClass(asset.changePercent ?? 0)}`}>
                    <div className={styles.heatTop}>
                      <span className={styles.heatSymbol}>{asset.symbol}</span>
                      {asset.name && <span className={styles.heatName}>{asset.name}</span>}
                    </div>
            ...
            <div className={styles.heatmapCard}>
              <div className={styles.panelHeader}>
                <h4 className={styles.visualTitle}>{language === "ko" ? "미국 주식 히트맵" : "US stock heatmap"}</h4>
                <span className={styles.panelSubtle}>{language === "ko" ? "대형주와 지수" : "mega caps and indexes"}</span>
              </div>
              <div className={styles.heatmapGrid}>
                {stockHeatmapAssets.map((asset, i) => (
                  <div key={asset.symbol} className={`${styles.heatTile} ${getHeatmapSizeClass(i)} ${getHeatmapToneClass(asset.changePercent ?? 0)}`}>
                    <div className={styles.heatTop}>
                      <span className={styles.heatSymbol}>{asset.symbol}</span>
                      {asset.name && <span className={styles.heatName}>{asset.name}</span>}
          ...
          <div className={styles.scenarioCard}>
            <div className={styles.panelHeader}>
              ...
              <span className={`${styles.signalTone} ${toneClass(todayScenario.tone)}`}>{todayScenario.tone}</span>
            </div>
      ...
      {/* Bottom row: Market Brief (merged, no duplicates) + Quick Ideas */}
        ...
        <article className={styles.panel}>
          ...
          <div className={styles.briefGrid}>
            <div className={styles.briefCard}>
              <span className={styles.briefLabel}>{language === "ko" ? "공포·탐욕 지수" : "Fear & Greed"}</span>
            ...
            <div className={styles.briefCard}>
              ...
              <p className={styles.briefMeta}>{featuredBriefing?.marketView || copy.marketViewFallback}</p>
            </div>
          </div>
        </article>
        ...
        <article className={styles.panel}>
          ...
          <div className={styles.promptList}>
            <div className={styles.promptCard}>
              <p className={styles.promptLabel}>{language === "ko" ? "미국주식 뉴스에서 보기 좋은 키워드" : "US stock news filters"}</p>
              <p className={styles.promptText}>ETF, Fed, AI, Tesla, NVIDIA, Earnings</p>
            </div>
            <div className={styles.promptCard}>
              <p className={styles.promptLabel}>{language === "ko" ? "한국주식 뉴스에서 보기 좋은 키워드" : "Korean stock news filters"}</p>
              <p className={styles.promptText}>코스피, 코스닥, 삼성전자, SK하이닉스, 반도체, 외국인</p>
            </div>
            <div className={styles.promptCard}>
              <p className={styles.promptLabel}>{language === "ko" ? "차트에서 바로 비교할 조합" : "Useful chart pairs"}</p>
